...
): Array<{ keyName: string; namespace: string; line: number }> {
  const keysMap = extractStaticKeysShared(code, '', defaultNamespace || 'ohjaaja', lines);
 ...
 */
function processDynamicKeys(code: string, fileName: string): Array<{ line: number; warning: string }> {
