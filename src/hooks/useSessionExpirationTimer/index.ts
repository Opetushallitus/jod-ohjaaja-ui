...
export const useSessionExpirationTimer = ({
  ...
  // Keep latest callbacks without re-running the interval effect on every render (avoids stale loginLink etc. in Root)
  const onWarningRef = React.useRef(onWarning);
  const onExpiredRef = React.useRef(onExpired);
  const onExtendedRef = React.useRef(onExtended);
  ...
  const extend = React.useCallback(async () => {
    ...
    await client.GET('/api/profiili/ohjaaja');
    resetNoteFlags();
    extendSession();
